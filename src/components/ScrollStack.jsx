...
const ScrollStack = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    const endElementTop = endElement ? getElementOffset(endElement, cardsRef.current.length) : 0;
  ...
  useLayoutEffect(() => {
    ...
    // Cache initial document offsets
    cardOffsetsRef.current = cards.map(card => {
      const rect = card.getBoundingClientRect();
      return rect.top + window.scrollY;
    });

    // Also cache the end element offset
    const endElement = useWindowScroll
      ? document.querySelector('.scroll-stack-end')
      : scroller?.querySelector('.scroll-stack-end');
    if (endElement) {
      const rect = endElement.getBoundingClientRect();
      cardOffsetsRef.current[cards.length] = rect.top + window.scrollY;
    }
